...
COPY --chown=gradle:gradle . /app/persistence-middleware
WORKDIR /app/persistence-middleware
...
FROM jdk AS local
COPY --from=gradle /app/persistence-middleware/build/libs/*.jar /app/server.jar
EXPOSE 8080
