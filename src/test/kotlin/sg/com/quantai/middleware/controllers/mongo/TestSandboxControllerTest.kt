...
import java.time.LocalDate
    ...
    class TestSandboxControllerUnitTest {
        ...
        @Test
        fun `executeSandboxStrategy builds URI with correct margin parameters`() {
            val mockWebClient = mock(WebClient::class.java)
            val mockPostSpec = mock(WebClient.RequestBodyUriSpec::class.java)
            val mockRequestBodySpec = mock(WebClient.RequestBodySpec::class.java)
            val mockResponseSpec = mock(WebClient.ResponseSpec::class.java)

            // Stub WebClient chain
            `when`(mockWebClient.post()).thenReturn(mockPostSpec)

            // Capture the URI builder lambda
            var capturedUri: URI? = null
            `when`(mockPostSpec.uri(any<Function<UriBuilder, URI>>())).thenAnswer { invocation ->
                val fn = invocation.arguments[0] as Function<UriBuilder, URI>
                capturedUri = fn.apply(UriComponentsBuilder.newInstance())
                mockRequestBodySpec
            }

            `when`(mockRequestBodySpec.bodyValue(any<Any>())).thenReturn(mockRequestBodySpec)
            `when`(mockRequestBodySpec.retrieve()).thenReturn(mockResponseSpec)
            `when`(mockResponseSpec.bodyToMono(String::class.java)).thenReturn(Mono.just("success"))

            val userId = "user1"
            val strategyId = "strategy1"
            val portfolioJson = mapOf("uid" to "p1", "assets" to emptyList<Any>())
            val strategyCode = "some code"
            val startDate: String? = null
            val endDate: String? = null
            val startingCash: Double? = null
            ...
            val queryParams = capturedUri!!.query.split("&").associate {
                val (k, v) = it.split("=")
                k to v
            }

            assertEquals("true", queryParams["marginEnabled"])
            assertEquals("0.5", queryParams["initialMarginRate"])

            verify(mockRequestBodySpec).bodyValue(
                mapOf("portfolio" to portfolioJson, "strategyCode" to strategyCode)
            )
