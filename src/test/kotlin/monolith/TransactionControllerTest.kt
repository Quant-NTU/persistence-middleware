...
import monolith.data.CryptoRepository
import monolith.data.Transaction
import monolith.data.TransactionRepository
import monolith.data.TransactionType
import monolith.data.TransactionStatus
...
import monolith.services.POJOS.CoinHistory.HistoryInstance
...
constructor(
    ...
    private fun saveOneTransaction(
            ...
            owner: User = userRepository.save(User("Name", "Email", "Password", "Salt"))
    ) =
    ...
    fun `should return all transactions from a user`() {
        ...
        val user1 = userRepository.save(User("Name1", "Email1", password1, salt1))
        val user2 = userRepository.save(User("Name2", "Email2", password2, salt2))
    ...
    fun `should return single transaction by id`() {
        ...
        val (password1, salt1) = hashAndSaltPassword("Password1")
        val user = userRepository.save(User("Name1", "Email1", password1, salt1))
    ...
    fun `should create new transaction add attach it to user`() {
        ...
        val user = userRepository.save(User("Name", "Email", password, salt))
        var userId = user.uid
        val crypto =
                cryptoRepository.save(
                        Crypto(
